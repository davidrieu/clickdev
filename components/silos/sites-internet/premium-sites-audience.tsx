...
import { SITES_INTERNET_AUDIENCE_CARDS } from '@/lib/constants/sites-internet-premium';
import { StellarField, useSectionStellarPointer } from './stellar-field';

/** « Pour qui je travaille » — grille identique à l’accueil (cartes + halo). */
export function PremiumSitesAudience() {
      ...
      <div className="relative z-10 mx-auto max-w-[1400px] px-4 md:px-8">
        ...
        >
          ...
          <h2 className="si-serif-display mt-4 text-[clamp(1.85rem,3.5vw,3rem)] font-medium tracking-[-0.03em] text-white">
            Pour qui je développe des sites
          </h2>
          <p className="mt-4 text-sm text-white/60 md:text-base">
            Des projets de 2 000 € à 200 000 €, avec la même rigueur.
          </p>
        </motion.div>
        <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:gap-5">
          {SITES_INTERNET_AUDIENCE_CARDS.map((card, index) => (
            <motion.div
              key={card.title}
