...
const ITEMS: BentoItem[] = [
  {
    ...
    title: 'iOS',
    description:
      'Swift, SwiftUI, guidelines Apple, TestFlight et passage App Store sans friction inutile.',
  },
  {
    href: '/applications-mobiles/android',
    title: 'Android',
    description: 'Kotlin, Compose, Play Console, flavors et qualité sur une vraie diversité d’appareils.',
  },
  {
    href: '/applications-mobiles/react-native',
    title: 'React Native',
    description: 'Une base pour iOS et Android, TypeScript, modules natifs quand le métier l’exige.',
  },
  {
    href: '/applications-mobiles/pwa',
    title: 'PWA',
    description: 'Service worker, manifest, offline ciblé : le web quand le store n’est pas le bon levier.',
  },
];
...
export function ApplicationsMobilesPremiumBento() {
      ...
      <div className="relative z-10 mx-auto max-w-[1400px] px-4 md:px-8">
        ...
        >
          ...
          <h2 className="si-serif-display mt-4 text-[clamp(2rem,4vw,3.5rem)] leading-[1.08] font-medium tracking-[-0.03em] text-white">
            Quatre façons d’attaquer le mobile — selon votre contexte
          </h2>
          <p className="mt-5 max-w-xl text-sm leading-relaxed text-white/60 md:text-base">
            Chaque lien mène à une page détaillée : même exigence de clarté, budgets et FAQ dédiés.
          </p>
        </motion.div>

        <div className="mt-16 grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-2">
          {ITEMS.map((item, i) => (
