...
type Props = { groups: readonly Group[] };

export function DevisIndexSections({ groups }: Props) {
  return (
    <div className="bg-black">
      <PremiumSectionDivider />
      {groups.map((group, gi) => (
        <DevisGroupSection key={group.id} group={group} index={gi} />
      ))}
    </div>
  );
}

function DevisGroupSection({ group, index }: { group: Group; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-[1200px] px-4 md:px-8">
        ...
        >
          <p className="font-mono text-[10px] tracking-[0.26em] text-white/40 uppercase">Bloc {index + 1}</p>
        ...
        <ul className="mt-12 grid list-none gap-4 p-0 sm:grid-cols-2 lg:gap-5">
          {group.items.map((item, i) => (
            ...
            >
              <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-white/[0.1] bg-gradient-to-br from-neutral-950 via-neutral-900/85 to-neutral-900 p-5 transition duration-500 hover:border-white/[0.18] hover:shadow-[0_24px_80px_-40px_rgba(46,8,207,0.35)]">
                <span className="font-mono text-[10px] tracking-[0.2em] text-white/35 uppercase">Point de vue</span>
                <h3 className="si-serif-display mt-2 text-lg font-medium tracking-[-0.02em] text-white">
                  {item.title}
                </h3>
